...
- **(a) Backend:** none new. Optionally add `GET /api/today` that bundles: latest `/api/health` day + today's planned workout (once the plan engine exists) + the latest readiness report snippet. For now, call `/api/health?limit=` (most-recent day) directly.
...
- **(b) Contract:** matches `STATE.trainingBlock` (`{name, range, week, totalWeeks, percentComplete, phases:[{week,label,setsPerMg,status}]}`).
...
- **(b) Contract:** matches `STATE.recoveryRules` (`[{text, detail, enabled}]`) — but store the structured condition so it can actually fire.
...
- **Body Mode is new:** "Cut / Maintain / Bulk" mode with weekly targets + a weight/bodyfat bias. `setMode` (`coach.js:118`) only flips local state.
...
- **(a) Backend — new:** model `NotificationPref`: `key`, `enabled` (e.g. `daily_plan`, `recovery_alert`, `plan_drift`, `weekly_review`). Endpoints `GET/PUT /api/notification-prefs`. The scheduler (`src/coach/scheduler.py`) + `notify.py` should check these before sending push. "Nudges" = derived from current state (low readiness, missed legs, review ready) — can be a `GET /api/nudges` computed endpoint or folded into `/api/today`.
